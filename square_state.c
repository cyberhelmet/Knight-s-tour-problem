...
*	Author: J.R.Dermoudy, Abhishek Tamang ( 551272 ) and Sandip Adhikari ( 555242 )
*	Version: May 2021
*
...
{
	trace("init_square_state: init_square_state starts"); //This is for debug purpose
	...
	*sp = (square_state)malloc(sizeof(struct square_state_int));
	...
	trace("init_square_state: init_square_state ends"); //This is for debug purpose
}
...
{
	trace("get_row_num: get_row_num starts and finishes"); //This is for debug purpose
...
{
	trace("get_column_num: get_column_num starts and finishes"); //This is for debug purpose

	return (s->column); //fetch and return current column number of the square
}
...
{
	trace("set_row_num: set_row_num starts"); //This is for debug purpose

	s->row = r;

	trace("set_row_num: set_row_num ends"); //This is for debug purpose
}
...
{
	trace("set_column_num: set_column_num starts"); //This is for debug purpose

	s->column = c; //updates the column field of the square_state s with the value of integer c

	trace("set_column_num: set_column_num ends"); //This is for debug purpose
}
...
{
	trace("get_count: get_count starts and finishes"); //This is for debug purpose
...
{
	trace("occupied: visit starts and finishes"); //This is for debug purpose

	return (s->visited); //checks and return the state of the square_state s, either it is occupied or not in boolean
}
...
{
	trace("occupy: occupy starts"); //This is for debug purpose
	s->visited = true;				//updates of value of the visited field of the square_state s to true
	s->move = c;					//updates the value of the integer c to the move field of the square_state s
	trace("occupy: occupy ends");	//This is for debug purpose
}
...
square_state clone_square_state(square_state s)
{
	...
	trace("clone_square_state: clone_square_state starts"); //This is for debug purpose

	init_square_state(&n, get_row_num(s), get_column_num(s));
	if (occupied(s))
	{
		trace("clone_square_state: occupying square on copy"); //This is for debug purpose
		occupy(n, get_count(s));
	}

	trace("clone_square_state: clone_square_state finishes"); //This is for debug purpose
...
{
	trace("show_square_state: show_square_state starts"); //This is for debug purpose
	...
	trace("show_square_state: show_square_state ends"); //This is for debug purpose
}
